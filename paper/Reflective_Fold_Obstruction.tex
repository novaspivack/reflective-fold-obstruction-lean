...
\textbf{Trust boundary:} Fold and hull theorems are internal to the Lean relation calculus formalized here; mapping to physical or institutional ``architectures'' is an external modeling task.

\NovaZenodoInAbstract
\end{abstract}

\noindent\textbf{Keywords:} reflective fold, internal closure, invariant hull, Lean~4, semantic types

\noindent\textbf{MSC 2020:} 03F40, 03B70, 68T27

\noindent\textbf{Code/Data availability:}\newline
\begingroup\raggedright
\url{https://github.com/novaspivack/reflective-fold-obstruction-lean}\linebreak
(reflective-fold-obstruction development; \path{lake build}; see repository \path{README} and \path{lakefile.lean}.)
\par\endgroup

\section{Premises, disagreement coordinates, and trust boundary}\label{sec:premises-rfo-flagship}
\begin{keybox}[title=Where disagreement can (and cannot) live]
\textbf{Formal spine.}
Obstruction lemmas are tied to \texttt{ReflTransGen} reachability and named preservation predicates in the artifact.

\medskip
\textbf{Premise coordinates (for external readers).}
\begin{itemize}[leftmargin=2em]
\item \textbf{Primitive-step choice.} Conclusions depend on the declared step relation~$r$ and its reflexive--transitive closure; changing~$r$ changes the hull.
  \emph{Logical effect:} Industrial workflow graphs are not automatically isomorphic to the formal~$r$ without a verified functor bridge.
\item \textbf{Semantic-type strand.} Turing-completeness claims concern standard computability predicates alongside tag preorders formalized in Lean; they do not subsume informal philosophy of mind without translation.
  \emph{Logical effect:} Accepting the computational lemma leaves metaphysical packaging optional.
\end{itemize}
\end{keybox}
...
\begin{keybox}[title={Computational vs.\ architectural ceilings}]
\textbf{Do not conflate them.}  Turing-completeness settles how much a standard model can
\emph{compute externally}; fold obstruction settles how far a fixed system can move
\emph{inside its own} primitive dynamics while preserving chosen invariants.  The
semantic-type layer (\Cref{sec:semantic-type}) proves the second can bound \emph{kinds of
being} (tags) even when the first is maximal---\textbf{instantiation along $r$ is not
...
\begin{keybox}[title={Diagonal pressure vs.\ dynamical hull vs.\ semantic types}]
\textbf{\RI{}}: no universal self-parametrization of the function space (Lawvere/diagonal).
\textbf{\RFO{}}: preserved invariants close internal hulls (fold, not iterate).
\textbf{Semantic types (\Cref{sec:semantic-type})}: the same hull story at the tag layer,
including provable \textbf{limits on Turing-complete hosts} that are not the halting
or Rice plotlines.  Keep the flavours separate: they reinforce each other, but none
reduces to the others.
\end{keybox}
...
\begin{keybox}[title={Turing-complete $\neq$ semantic-type complete}]
\textbf{Orthogonal limits.}  A host may run every partial recursive function in the usual
sense and still be \emph{permanently type-bounded}: its primitive step can preserve
predicates that make some tag pairs unreachable by any internal chain.  \textbf{Simulation
does not repair the gap}: the host may encode a counterfactual machine that
``would'' change type, without itself \emph{instantiating} that transition natively.  What is
machine-checked is dynamical obstruction---\textbf{hulls closed by preserved
invariants}---not a substitute for Rice or the halting problem.
\end{keybox}
...
\begin{keybox}[title={Tags inherit the fold boundary}]
Semantic tagging is not a separate metaphysics: it is \Cref{thm:fold-obstruction} with
predicates indexed by tags.  \textbf{Preserved tag extensions} mean that every internal
route stays inside the invariant class of each tag it touches; \textbf{type gaps} are
mappings incompatible with that preservation.  The move from states to tags packages the
same obstruction for comparing \emph{architectural kinds}.
\end{keybox}
...
\begin{keybox}[title={The type preorder is provably nontrivial}]
\textbf{Not all tags are comparable by internal iteration.}  The artifact gives concrete
witness families: self-modeling \emph{depth} on $\mathbb{N}$ (identity dynamics), abstract
\emph{adjudication class} under preserved kind, and the reflective \emph{sort} gap
(object branch vs.\ morphism carrier) aligned with the RI bridge.  Together they nail
down that \textbf{semantic-type reachability is genuinely nontrivial as a preorder}, not a
vacuous afterthought to Turing completeness.
\end{keybox}
...
\begin{keybox}[title={Simulation vs.\ realization (machine-checked split)}]
\textbf{Forward half is free.}  Typed primitive simulations always \emph{project} type
reachability to the codomain; type gaps on the specification \emph{pull back} to any
simulating dynamics.
\textbf{The converse costs structure.}  Without backward step-lifting, the host may
simulate without realizing every codomain trajectory.  A section $\sigma$ with
$\pi\circ\sigma=\mathrm{id}$ that lifts each primitive step backward
(\texttt{TypedPrimitiveSimulationSection}) is the minimal repair: then \textbf{type
reachability matches the pullback iff it matches the model}, and the same for packaged
obstruction (\texttt{typeReachable\_pullback\_iff\_of\_section}).  \textbf{Realization =
bidirectional agreement}, not ``bigger state space.''
\end{keybox}
...
\begin{keybox}[title={One-sided preservation is often enough}]
Many arguments fix a predicate $P$ whose forward-closure is cheap, without assuming a full
bipartite discipline in which $\neg P$ is also step-closed.  \textbf{Complementary-tag gaps}
(\texttt{typeGap\_of\_complementary\_tags}) need only that $P$ be preserved along primitive
steps, with two tags classifying $P$ and $\neg P$.  \textbf{Seed-local hull exclusion}
(\texttt{false\_tag\_not\_mem\_tagsOnHull\_of\_preserved\_seed}) shows a $\neg P$-tag never
appears on the hull from $P$-seeds under the same hypothesis.  These lemmas target the
\emph{reachable component from realistic initial conditions}, not a global state space.
\end{keybox}
...
\noindent
The development lives under\\
\leanref{ReflectiveFoldObstruction/SemanticType/}\\
with stable import surface \leanref{Core/SemanticTypeObstruction}.

\smallskip
\noindent\textbf{Core.}
\begin{itemize}[nosep,leftmargin=1.5em]
  \item \leanref{semanticType\_preorder\_nontrivial}
  \item \leanref{selfModelDepth\_obstruction}
  \item \leanref{adjudication\_semantic\_obstruction}
  \item \leanref{RI\_semantic\_type\_mismatch}
\end{itemize}

\noindent\textbf{Simulation / realization.}
\begin{itemize}[nosep,leftmargin=1.5em]
  \item \leanref{typeGap\_simulation\_pullback}
  \item \leanref{typeReachable\_pullback\_iff\_of\_section}
  \item \leanref{SemanticTypeObstruction\_pullback\_iff\_of\_section}
\end{itemize}

\noindent\textbf{Complementary / hull.}
\begin{itemize}[nosep,leftmargin=1.5em]
  \item \leanref{typeGap\_of\_complementary\_tags}
  \item \leanref{false\_tag\_not\_mem\_tagsOnHull\_of\_preserved\_seed}
\end{itemize}

\noindent See also \Cref{tab:anchors}.
...
\begin{keybox}[title={Closing moral}]
\textbf{Internal closure is not architectural universality.}  Iteration along a chosen
primitive dynamics respects preserved invariants; crossing an invariant boundary is a
\emph{fold}.  Classical computability fixes the \emph{computational} ceiling; this
development fixes a complementary \emph{semantic-dynamical} ceiling---including a proved
distinction between what can be \emph{simulated forward} and what is \emph{realized} with
backward-lifting structure.  \textbf{Three orthogonal stories} work together: \RFO{} folds,
\RI{} diagonal pressure, and semantic-type obstruction along~$r$---each machine-checked,
each with its own impossibility flavour.
\end{keybox}
