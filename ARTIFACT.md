...
**Semantic type obstruction (`SPEC_020_H3T`, 2026-03-29):** `SemanticType/` — bundled **`Typing`**, **type reachability** / **gaps**, three instances (self-model depth, adjudication kind, RI mor-carrier vs obj), **`semanticType_preorder_nontrivial`**; **corollaries:** primitive-step extension monotonicity, **`tagsOnHull`** (incl. **`false_tag_not_mem_tagsOnHull_of_preserved_seed`**), **pair-tag** obstruction, **`BipartitionDyn`** / **`FoldObstructionBundle`**, **typed simulation** + **section / realisation** (**`typeReachable_pullback_iff_of_section`**, obstruction **iff**), one-sided **complementary tags** (**`typeGap_of_complementary_tags`**), infinite depth **antichain**, computability scope note; **`reflTransGen_map`** in `InternalOps`; stable import **`Core/SemanticTypeObstruction`**; flagship paper §9.
